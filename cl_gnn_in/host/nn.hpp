											...
											cl_mem BUF_RM_x1, cl_mem BUF_RM_x2, cl_mem BUF_RM_x4, cl_mem BUF_RM_x_arr_t)
{
	...
	linear(BUF_RM_x_arr_t, BUF_RM_WEIGHT_0, BUF_RM_BIAS_0, BUF_RM_x1, x_h, x_w, RM_w0, "relu");
	linear(BUF_RM_x1, BUF_RM_WEIGHT_2, BUF_RM_BIAS_2, BUF_RM_x2, x_h, RM_w0, RM_w2, "relu");
	linear(BUF_RM_x2, BUF_RM_WEIGHT_4, BUF_RM_BIAS_4, BUF_RM_x4, x_h, RM_w2, RM_w4, "relu");
	linear(BUF_RM_x4, BUF_RM_WEIGHT_6, BUF_RM_BIAS_6, BUF_x_out, x_h, RM_w4, RM_w6, "sigmoid");
}
									...
									cl_mem BUF_OM_x_arr_t, cl_mem BUF_OM_x1, cl_mem BUF_OM_x2)
{
	...
	linear(BUF_OM_x_arr_t, BUF_OM_WEIGHT_0, BUF_OM_BIAS_0, BUF_OM_x1, x_h, x_w, OM_w0, "relu");
	linear(BUF_OM_x1, BUF_OM_WEIGHT_2, BUF_OM_BIAS_2, BUF_OM_x2, x_h, OM_w0, OM_w2, "relu");
	linear(BUF_OM_x2, BUF_OM_WEIGHT_4, BUF_OM_BIAS_4, BUF_x_out, x_h, OM_w2, OM_w4, "None");
}
...
vector<vector<vector<float>>> ndr_forward(vector<vector<vector<float>>> obj,
	...
	//cout << term_w << " , " << term_h << "\n ------------------ \n";
	//int size = term_w*term_h;
	//cl_int status;
	//float check[size];
	//read_out_buffer(BUF_interaction_term_arr, check, size, status);
...
vector<vector<vector<float>>> swi_forward(vector<vector<vector<float>>> obj,
																			vector<vector<vector<float>>> sr,
																			vector<vector<vector<float>>> rr,
																			vector<vector<vector<float>>> ri){
	vector<vector<float>> predict;
	...
	swi_IN(
	  BUF_obj_arr, BUF_obj_arr_t, BUF_sr_arr, BUF_sender_arr, BUF_rr_arr, BUF_ri_arr,
		BUF_receiver_arr, BUF_effect_receiver_arr, BUF_effect_receiver_arr_t,
		BUF_predict_arr, BUF_agg_arr, BUF_inf_arr, BUF_interaction_term_arr,
		BUF_effect_arr, BUF_pred_arr, BUF_RM_x1, BUF_RM_x2, BUF_RM_x4, BUF_RM_x_arr_t,
		BUF_OM_x1, BUF_OM_x2, BUF_OM_x_arr_t, BUF_RM_WEIGHT_0, BUF_RM_WEIGHT_2,
		BUF_RM_WEIGHT_4, BUF_RM_WEIGHT_6, BUF_OM_WEIGHT_0, BUF_OM_WEIGHT_2,
		BUF_OM_WEIGHT_4, BUF_RM_BIAS_0, BUF_RM_BIAS_2, BUF_RM_BIAS_4, BUF_RM_BIAS_6,
		BUF_OM_BIAS_0, BUF_OM_BIAS_2, BUF_OM_BIAS_4,
	  obj_w, obj_h, sr_w, sr_h, ri_w, ri_h, rr_w, rr_h, obj_t_w,
		obj_t_h, sender_w, sender_h, receiver_w, receiver_h, term_w,
		term_h, effect_w, effect_h, effect_receiver_w, effect_receiver_h,
		aggregate_w, aggregate_h, pred_w, pred_h, out_w, out_h, RM_w0,
		RM_w2, RM_w4, RM_w6, OM_w0, OM_w2, OM_w4);
	read_out_buffer(BUF_pred_arr, pred_arr, out_w*out_h, status);
	predict = array2_2dvec(pred_arr, out_w, out_h);
	int m = predict.size();
	predicted.push_back(predict);
